...
class SlackClient(object):
    ...
    def get_config_groups(self, groups_conf, groups_pillar_name):
        ...
        try:
            groups_gen = itertools.chain(list(self._groups_from_pillar(groups_pillar_name).items()), list(use_groups.items()))
    ...
    def can_user_run(self, user, command, groups):
        ...
        for key, val in list(groups.items()):
            ...
            if (command not in val['commands']) and (command not in list(val.get('aliases', {}).keys())):
    ...
    def control_message_target(self, slack_user_name, text, loaded_groups, trigger_string):
        ...
        if cmdline[0] in list(permitted_group[1].get('aliases', {}).keys()):
    ...
    def run_commands_from_slack_async(self, message_generator, fire_all, tag, control, interval=1):
        ...
        while True:
            ...
            for msg in message_generator:
                ...
                log.trace('Got a message from the generator: %s', list(msg.keys()))
                if count > 10:
            ...
            job_status = self.get_jobs_from_runner(list(outstanding.keys()))  # dict of job_ids:results are returned
