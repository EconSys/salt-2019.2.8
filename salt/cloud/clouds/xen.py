...
def list_nodes_full(session=None):
    ...
    for vm in vms:
        ...
        if not record['is_a_template'] and not record['is_control_domain']:
            ...
            if 'snapshot_time' in list(vm_cfg.keys()):
...
def _determine_resource_pool(session, vm_):
    ...
    '''
    resource_pool = ''
    if 'resource_pool' in list(vm_.keys()):
...
def _determine_storage_repo(session, resource_pool, vm_):
    ...
    '''
    storage_repo = ''
    if 'storage_repo' in list(vm_.keys()):
...
def _set_static_ip(name, session, vm_):
    ...
    ipv4_cidr = ''
    ipv4_gw = ''
    if 'ipv4_gw' in list(vm_.keys()):
        log.debug('ipv4_gw is found in keys')
        ipv4_gw = vm_['ipv4_gw']
    if 'ipv4_cidr' in list(vm_.keys()):
