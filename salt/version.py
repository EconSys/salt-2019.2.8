...
class SaltStackVersion(object):
    ...
    LNAMES = dict((k.lower(), v) for (k, v) in iter(list(NAMES.items())))
    VNAMES = dict((v, k) for (k, v) in iter(list(NAMES.items())))
    RMATCH = dict((v[:2], k) for (k, v) in iter(list(NAMES.items())))
