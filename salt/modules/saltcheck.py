...
def run_state_tests(state):
    ...
    for state_name in sls_list:
        ...
        stl.load_test_suite()
        results_dict = {}
        for key, value in list(stl.test_dict.items()):
    ...
    for state in results:
        if len(list(results[state].items())) == 0:
            missing_tests = missing_tests + 1
        else:
            for dummy, val in list(results[state].items()):
    ...
    def load_file(self, filepath):
        ...
        try:
            with __utils__['files.fopen'](filepath, 'r') as myfile:
                ...
                for key, value in list(contents_yaml.items()):
