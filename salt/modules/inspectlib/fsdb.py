...
class CsvDB(object):
    ...
    def store(self, obj, distinct=False):
        ...
        '''
        if distinct:
            fields = dict(list(zip(list(self._tables[obj._TABLE].keys()),
    ...
    def __criteria(self, obj, matches=None, mt=None, lt=None, eq=None):
        ...
        for field, value in list((mt or {}).items()):
            if getattr(obj, field) <= value:
                return False

        # Fail matcher if "more than"
        for field, value in list((lt or {}).items()):
            if getattr(obj, field) >= value:
                return False

        # Fail matcher if "not equal"
        for field, value in list((eq or {}).items()):
            if getattr(obj, field) != value:
                return False

        # Fail matcher if "doesn't match"
        for field, value in list((matches or {}).items()):
    ...
    def _to_type(self, data, type):
        if type == 'int':
            ...
            data = sys.version_info[0] == 2 and int(data) or int(data)  # pylint: disable=W1699
        else:
            data = str(data)
        return data
