...
def _arg2opt(arg):
    ...
    '''
    res = [o for o, a in list(option_toggles.items()) if a == arg]
    res += [o for o, a in list(option_flags.items()) if a == arg]
...
def _parse_options(entry, options, include_unset=True):
    ...
    if include_unset:
        # toggle optioons
        for name in list(option_toggles.values()):
            if name not in log_cfg:
                log_cfg[name] = False

        # flag options
        for name in list(option_flags.values()):
            if name not in log_cfg:
                log_cfg[name] = None
...
def show_args():
    ...
    for flag, arg in list(option_toggles.items()):
        mapping['flags'][flag] = arg
    for option, arg in list(option_flags.items()):
...
def rotate(name, pattern=None, conf_file=default_conf, **kwargs):
    ...
    for arg, val in list(kwargs.items()):
        if arg in list(option_toggles.values()) and val:
            command = "{} {}".format(
                command,
                _arg2opt(arg),
            )
        elif arg in list(option_flags.values()):
            command = "{} {} {}".format(
                command,
                _arg2opt(arg),
