...
def cache_id(service, name, sub_resource=None, resource_id=None,
    ...
    if invalidate:
        if cxkey in __context__:
            ...
            return True
        elif resource_id in list(__context__.values()):
            ctx = dict((k, v) for k, v in list(__context__.items()) if v != resource_id)
            __context__.clear()
...
def ordered(obj):
    if isinstance(obj, (list, tuple)):
        ...
        return dict((six.text_type(k) if isinstance(k, six.string_types) else k, ordered(v)) for k, v in list(obj.items()))
