        ...
        if not self.__upgrade_codes or self.__reg_key_guid not in self.__upgrade_codes:
            ...
            try:
                ...
                if exc.winerror == winerror.ERROR_FILE_NOT_FOUND:
                    ...
                    return ''
                raise
            squid_upgrade_code_all, _, _, suc_pytime = list(zip(*win32api.RegEnumKeyEx(uc_handle)))  # pylint: disable=no-member
    ...
    def list_patches(self):
        ...
        if self.__patch_list is None:
            ...
            if not pc_sub_key_cnt:
                return []
            squid_patch_all, _, _, _ = list(zip(*win32api.RegEnumKeyEx(pat_all_handle)))  # pylint: disable=no-member
...
class WinSoftware(object):
    ...
    def __get_software_details(self, user_pkgs):
        ...
        for arch_flag in arch_list:
            ...
            reg_key_all, _, _, _ = list(zip(*win32api.RegEnumKeyEx(handle)))  # pylint: disable=no-member
            win32api.RegCloseKey(handle)  # pylint: disable=no-member
