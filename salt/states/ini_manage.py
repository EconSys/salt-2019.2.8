...
def options_present(name, sections=None, separator='=', strict=False):
    ...
    try:
        ...
        if sections:
            options = {}
            for sname, sbody in list(sections.items()):
            ...
            original_sections = {}
            for key, val in list(cur_ini.items()):
                ...
                for opt_to_remove in set(original_top_level_opts).difference(options):
                    if __opts__['test']:
                        ...
                        changes.update({opt_to_remove: {'before': original_top_level_opts[opt_to_remove],
                                                        'after': None}})
            for section_name, section_body in [(sname, sbody) for sname, sbody in list(sections.items())
    ...
    if 'error' in changes:
        ...
        for ciname, body in list(changes.items()):
            if body:
