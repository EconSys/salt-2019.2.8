...
def baseline_snapshot(name, number=None, tag=None, include_diff=True, config='root', ignore=None):
    ...
    for target in ignore:
        if os.path.isfile(target):
            ...
            for target_file in [target_file for target_file in list(status.keys()) if target_file.startswith(target)]:
    ...
    if __opts__['test'] and status:
        ret['changes'] = status
        ret['comment'] = "{0} files changes are set to be undone".format(len(list(status.keys())))
        ret['result'] = None
