...
class LoadAuth(object):
    ...
    def __auth_call(self, load):
        ...
        _load = {key: value for (key, value) in list(load.items()) if key in _valid}
...
class Resolver(object):
    ...
    def cli(self, eauth):
        ...
        if fstr not in self.auth:
            ...
            print("Available eauth types: {0}".format(", ".join(list(self.auth.file_mapping.keys()))))
            return ret
