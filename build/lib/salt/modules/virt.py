...
def _parse_qemu_img_info(info):
    ...
    for disk_infos in raw_infos:
        ...
        if 'full-backing-filename' in list(disk_infos.keys()):
            disk['backing file'] = format(disk_infos['full-backing-filename'])

        if 'snapshots' in list(disk_infos.keys()):
            disk['snapshots'] = [
                    {
                        'id': snapshot['id'],
                        'tag': snapshot['name'],
    ...
    for disk in disks:
        if 'backing file' in list(disk.keys()):
            candidates = [info for info in disks if 'file' in list(info.keys()) and info['file'] == disk['backing file']]
            if candidates:
...
def _disk_profile(profile, hypervisor, disks=None, vm_name=None, image=None, pool=None, **kwargs):
    ...
    if profile:
        ...
        disklist = [dict(d, name=name) for disk in disklist for name, d in list(disk.items())]
...
def init(name,
    ...
    if not hypervisor:
        ...
        if hypervisor is not None:
            ...
            hypervisors = sorted({x for y in [list(guest['arch']['domains'].keys()) for guest in caps['guests']] for x in y})
