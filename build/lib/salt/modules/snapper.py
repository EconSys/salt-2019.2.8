...
def _snapshot_to_data(snapshot):
    ...
    data['userdata'] = {}
    for key, value in list(snapshot[7].items()):
...
def set_config(name='root', **kwargs):
    ...
    try:
        data = dict((k.upper(), _config_filter(v)) for k, v in
                    list(kwargs.items()) if not k.startswith('__'))
...
def run(function, *args, **kwargs):
    ...
    func_kwargs = dict((k, v) for k, v in list(kwargs.items()) if not k.startswith('__'))
    kwargs = dict((k, v) for k, v in list(kwargs.items()) if k.startswith('__'))
...
def changed_files(config='root', num_pre=None, num_post=None):
    ...
    '''
    return list(status(config, num_pre, num_post).keys())
