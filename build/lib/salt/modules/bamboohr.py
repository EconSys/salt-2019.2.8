...
def list_employees(order_by='id'):
    ...
    for cat in directory:
        if cat.tag != 'employees':
            continue
        for item in cat:
            emp_id = list(item.items())[0][1]
            emp_ret = {'id': emp_id}
            for details in item.getchildren():
                emp_ret[list(details.items())[0][1]] = details.text
...
def show_employee(emp_id, fields=None):
    ...
    ret = {'id': emp_id}
    for item in items:
        ret[list(item.items())[0][1]] = item.text
    return ret
...
def update_employee(emp_id, key=None, value=None, items=None):
    ...
    return show_employee(emp_id, ','.join(list(items.keys())))
