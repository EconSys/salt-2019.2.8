...
def _create_networking_config(networks):
    ...
    return _client_wrapper('create_networking_config',
        {k: _client_wrapper('create_endpoint_config', **v) for k, v in list(networks.items())})
...
def call(name, function, *args, **kwargs):
    ...
    try:
        salt_argv = [
            ...
            '--',
            function
        ] + list(args) + ['{0}={1}'.format(key, value) for (key, value) in list(kwargs.items()) if not key.startswith('__')]

        ret = run_all(name, subprocess.list2cmdline(list(map(str, salt_argv))))
        # python not found
        if ret['retcode'] != 0:
