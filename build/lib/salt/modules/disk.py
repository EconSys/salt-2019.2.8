...
def _parse_numbers(text):
    ...
    try:
        ...
        if text[-1] in list(postPrefixes.keys()):
...
def hpa(disks, size=None):
    ...
    hpa_data = {}
    for disk, data in list(hdparms(disks, 'N').items()):
    ...
    for disk, data in list(hpa_data.items()):
        try:
...
def smart_attributes(dev, attributes=None, values=None):
    ...
    for line in smart_result:
        ...
        data = dict(list(zip(fields, line[1:])))
        try:
            del data['_']
        except Exception:
            pass
...
def _iostats_dict(header, stats):
    ...
    stats = dict(list(zip(header, stats)))
    return stats
...
def _iostat_fbsd(interval, count, disks):
    ...
    if h_len < len(dev_header):
        ...
        for disk, stats in list(dev_stats.items()):
...
def _iostat_aix(interval, count, disks):
    ...
    for line in __salt__['cmd.run'](iostat_cmd).splitlines():
        ...
        if not re.match(r'\s', line):
            ...
            if disk_name not in list(dev_stats.keys()):
                dev_stats[disk_name] = []
        ...
        if ':' in line:
            ...
            if disk_mode not in list(dev_stats[disk_name][0].keys()):
    ...
    for disk, list_modes in list(dev_stats.items()):
        iostats[disk] = {}
        for modes in list_modes:
            for disk_mode in list(modes.keys()):
