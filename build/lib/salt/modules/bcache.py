...
def attach_(dev=None):
    ...
    if dev is None:
        res = {}
        for dev, data in list(status(alldevs=True).items()):
            if 'cache' in data:
                res[dev] = attach_(dev)
...
def config_(dev=None, **kwargs):
    ...
    updates = dict([(key, val) for key, val in list(kwargs.items()) if not key.startswith('__')])
...
def _sysfs_parse(path, base_attr=None, stats=False, config=False, internals=False, options=False):
    ...
    for modt, modlist in list(mods.items()):
        found = []
        if modt not in bintf:
            continue
        for mod in modlist:
            for intflist in list(bintf.values()):
                if mod in intflist:
                    found.append(mod)
                    intflist.remove(mod)
        bintf[modt] += found
    ...
    # -------- Fetch SysFS vals --------
    bintflist = [intf for iflist in list(bintf.values()) for intf in iflist]
    ...
    bresult = {}
    for iftype, intflist in list(bintf.items()):
        ifres = {}
...
def _alltrue(resdict):
    if resdict is None:
        return True
    return len([val for val in list(resdict.values()) if val]) > 0
