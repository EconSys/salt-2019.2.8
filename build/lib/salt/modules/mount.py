...
class _fstab_entry(object):
    ...
    def dict_from_line(cls, line, keys=fstab_keys):
        ...
        return dict(list(zip(keys, comps)))
...
class _vfstab_entry(object):
    ...
    def dict_from_line(cls, line):
        ...
        return dict(list(zip(cls.vfstab_keys, comps)))
...
def set_fstab(
    ...
    try:
        ...
        invalid_keys = list(filter(filterFn, match_on))
...
def filesystems(config='/etc/filesystems'):
    ...
    if ret_dict:
        ret_key = next(iter(list(ret_dict.keys())))
