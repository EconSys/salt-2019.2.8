...
def info_installed(*names, **kwargs):
    ...
    ret = dict()
    for pkg_name, pkgs_nfo in list(__salt__['lowpkg.info'](*names, **kwargs).items()):
...
def list_repo_pkgs(*args, **kwargs):
    ...
    for node in __zypper__.xml.call('se', '-s', *targets).getElementsByTagName('solvable'):
        pkginfo = dict(list(node.attributes.items()))
        try:
...
def search(criteria, refresh=False, **kwargs):
    ...
    for solvable in solvables:
        ...
        for k, v in list(solvable.attributes.items()):
...
def list_products(all=False, refresh=False):
    ...
    for prd in product_list[0].getElementsByTagName('product'):
        p_nfo = dict()
        for k_p_nfo, v_p_nfo in list(prd.attributes.items()):
...
def diff(*paths):
    ...
    if pkg_to_paths:
        local_pkgs = __salt__['pkg.download'](*list(pkg_to_paths.keys()))
        ...
        if kwargs.get('resolve_capabilities', False):
            try:
                ...
                try:
                    ...
                    if len(result) == 1:
                        name = next(iter(list(result.keys())))
                    elif len(result) > 1:
