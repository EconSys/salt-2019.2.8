...
def get_agent_settings():
    ...
    '''
    ret = dict()
    sorted_types = sorted(list(_SERVICE_TYPES.items()), key=lambda x: (-x[1], x[0]))
...
def set_agent_settings(contact=None, location=None, services=None):
    ...
    _LOG.debug('Agent settings configured successfully: %s', list(settings.keys()))
    return True
...
def set_community_names(communities):
    ...
    for vname in communities:
        ...
        try:
            ...
            message = (
                ...
                "{1}").format(communities[vname], list(_PERMISSION_TYPES.keys()))
            raise SaltInvocationError(message)
        values[vname] = vdata
    ...
    if failed_communities:
        ...
        return False
    _LOG.debug('Communities configured successfully: %s', list(communities.keys()))
    return True
