...
def set_container_setting(name, container, settings):
    ...
    current_settings = get_container_setting(
        name=name, container=container, settings=list(settings.keys()))
    ...
    for setting in settings:
        ...
        if setting == 'processModel.identityType' and settings[setting] in list(identityType_map2numeric.keys()):
    ...
    for setting in settings:
        ...
        if setting == 'processModel.identityType' and settings[setting] in list(identityType_map2string.keys()):
    ...
    log.debug('Settings configured successfully: %s', list(settings.keys()))
    return True
...
def set_webapp_settings(name, site, settings):
    ...
    current_settings = get_webapp_settings(
        name=name, site=site, settings=list(settings.keys()))
    ...
    log.debug('Settings configured successfully: {0}'.format(list(settings.keys())))
    return True
