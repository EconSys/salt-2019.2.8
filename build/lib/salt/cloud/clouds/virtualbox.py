...
def map_clonemode(vm_info):
    ...
    if vm_info['clonemode'] in mode_map:
        ...
        raise SaltCloudSystemExit(
            "Illegal clonemode for virtualbox profile.  Legal values are: {}".format(','.join(list(mode_map.keys())))
        )
...
def create(vm_info):
    ...
    wait_for_pattern = vm_info['waitforpattern'] if 'waitforpattern' in list(vm_info.keys()) else None
    interface_index = vm_info['interfaceindex'] if 'interfaceindex' in list(vm_info.keys()) else 0
