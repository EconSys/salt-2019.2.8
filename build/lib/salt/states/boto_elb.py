...
def _policies_present(name, policies, policies_from_pillar, listeners, backends,
    ...
    expected_policy_names = list(policies_by_cname.keys())
            ...
            if __opts__['test']:
                if tags_to_add:
                    msg = 'The following tag{0} set to be added: {1}.'.format(
                            ('s are' if len(list(tags_to_add.keys())) > 1 else ' is'),
                            ', '.join(list(tags_to_add.keys())))
                    ret['comment'] = ' '. join([ret['comment'], msg])
                    ret['result'] = None
                if tags_to_update:
                    msg = 'The following tag {0} set to be updated: {1}.'.format(
                            ('values are' if len(list(tags_to_update.keys())) > 1 else 'value is'),
                            ', '.join(list(tags_to_update.keys())))
                    ret['comment'] = '  '.join([ret['comment'], msg])
            else:
