...
def peered(name):
    ...
    if peers and any(name in v['hostnames'] for v in list(peers.values())):
        ret['result'] = True
    ...
    if newpeers and any(name in v['hostnames'] for v in list(newpeers.values())):
        ret['result'] = True
...
def add_volume_bricks(name, bricks):
    ...
    current_bricks = [brick['path'] for brick in list(volinfo[name]['bricks'].values())]
    ...
    if bricks_added:
        ...
        new_bricks = [brick['path'] for brick in list(__salt__['glusterfs.info']()[name]['bricks'].values())]
