...
def _get_libvirt_enum_string(prefix, value):
    ...
    sub_prefixes = [p for p, count in list(counts.items()) if count > 1 or (p.endswith('_') and p[:-1] in prefixes)]
...
def _callbacks_cleanup(cnx, callback_ids):
    ...
    '''
    for obj, ids in list(callback_ids.items()):
...
def start(uri=None,
    ...
    try:
        ...
        for obj, event_defs in list(CALLBACK_DEFS.items()):
