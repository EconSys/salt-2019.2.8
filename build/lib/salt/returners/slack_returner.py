...
def returner(ret):
    ...
    if changes is True:
        returns = {(key, value) for key, value in list(returns.items()) if value['result'] is not True or value['changes']}

    if only_show_failed is True:
        returns = {(key, value) for key, value in list(returns.items()) if value['result'] is not True}
