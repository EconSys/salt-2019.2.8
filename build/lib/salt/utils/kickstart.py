...
def mksls(src, dst=None):
        ...
        for line in fh_:
            ...
            if mode == 'command':
                if line.startswith('auth ') or line.startswith('authconfig '):
                    ...
                    if 'logvol' not in list(ks_opts.keys()):
                        ks_opts['logvol'] = []
            ...
            if line.startswith('%packages'):
                mode = 'packages'
                if 'packages' not in list(ks_opts.keys()):
    ...
    if 'utc' in list(ks_opts['timezone'].keys()):
        sls[ks_opts['timezone']['timezone']]['timezone'].append('utc')

    # Set network
    if 'network' in list(ks_opts.keys()):
        for interface in ks_opts['network']:
            ...
            if device is not None:
                ...
                if 'onboot' in list(interface.keys()):
                ...
                if 'noipv4' in list(interface.keys()):
                    sls[device]['ipv4'] = {'enabled': False}
                    del interface['noipv4']
                if 'noipv6' in list(interface.keys()):
    ...
    # Set selinux
    if 'selinux' in list(ks_opts.keys()):
