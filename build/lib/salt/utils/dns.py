...
class RFC(object):
    ...
    def validate(lookup, ref, match=None):
        if lookup in ref:
            ...
            return [code for code, name in list(ref.items()) if lookup in name][-1]
        else:
            # OrderedDicts only!(?)
            return list(ref.keys())[list(ref.values()).index(lookup)]
...
def _data2rec(schema, rec_data):
    ...
    try:
        ...
        if len(schema) == 1:
            res = _cast(rec_fields[0], next(iter(list(schema.values()))))
        else:
            res = dict((
                (field_name, _cast(rec_field, rec_cast))
                for (field_name, rec_cast), rec_field in zip(list(schema.items()), rec_fields)
            ))
        return res
...
def _data2rec_group(schema, recs_data, group_key):
    ...
    try:
        for rdata in recs_data:
            ...
            if len(rdata) == 1:
                rdata = next(iter(list(rdata.values())))
...
def services(services_file='/etc/services'):
    ...
    for svc, data in list(res.items()):
        if len(data) == 1:
            res[svc] = list(data.values()).pop()
            continue
        else:
