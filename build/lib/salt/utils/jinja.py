...
def show_full_context(ctx):
    return salt.utils.data.simple_types_filter({key: value for key, value in list(ctx.items())})
        ...
        def recurse_tree(xmliter, element=None):
            ...
            for tag, attrs in xmliter:
                ...
                if isinstance(attrs, dict):
                    sub.attrib = {attr: six.text_type(val) for attr, val in list(attrs.items())
