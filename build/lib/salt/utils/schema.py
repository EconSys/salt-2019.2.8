...
class Schema(six.with_metaclass(SchemaMeta, object)):
    ...
    def defaults(cls):
        ...
        defaults = {}
        for name, details in list(serialized['properties'].items()):
            if 'default' in details:
                defaults[name] = details['default']
                continue
            if 'properties' in details:
                for sname, sdetails in list(details['properties'].items()):
...
class DictItem(BaseSchemaItem):
    ...
    def __validate_attributes__(self):
        ...
        if self.pattern_properties is not None:
            ...
            for key, prop in list(self.pattern_properties.items()):
                if not isinstance(prop, (Schema, SchemaItem)):
                    raise RuntimeError(
...
class DefinitionsSchema(Schema):
    ...
    def serialize(cls, id_=None):
        ...
        aux_items = list(cls._items.values())
        ...
        while aux_items:
            ...
            if isinstance(item, OneOfItem):
                ...
                if item.properties:
                    aux_items.extend(list(item.properties.values()))
